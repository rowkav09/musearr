...
### Issue 5: Port Clash Vulnerability (Very Common Ports 3000 & 4000)
- **Severity**: Low
- **Affected Component/Files**: `docker-compose.yml`, `docker-compose.release.yml`, `.env.example`, `packages/config/src/index.ts`, `infra/caddy/Caddyfile`
- **Description**: Relying on ports 3000 and 4000 as default exposure ports conflicts heavily with existing developer servers and other self-hosted web software on local machines.
- **Reproduction or Evidence**: Starting Musearr while another Next.js, React, or local development server runs on port 3000 triggers "port already in use" errors.
- **Expected Behaviour**: Safe, custom-allocated and non-clashing default port numbers (e.g. 5530 and 5540).
- **Suggested Fix**: Remap all occurrences and default configurations to port `5530` (Web Dashboard) and `5540` (API Server).
- **Verification Status**: Fixed and verified.
- **Whether an existing PR already addresses it**: No.
