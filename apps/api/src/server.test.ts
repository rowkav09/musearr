...
describe('sync run observability', () => {
  ...
  async function ownerHeaders(app: ReturnType<typeof buildServer>) {
    await app.ready()
    return { cookie: `musearr_session=${app.jwt.sign({ sub: 'owner-id', role: 'owner' })}` }
  }
  ...
  it('returns a validated sync run by id and hides absent records', async () => {
    const database = vi
      .fn()
